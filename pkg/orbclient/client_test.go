...
func TestGetAnchorOrigin(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		vcBytes, err := buildCredential(payload).MarshalJSON()
	...
	t.Run("error - failed to read core index file", func(t *testing.T) {
		...
		require.Contains(t, err.Error(),
			"error reading core index file: retrieve CAS content at uri[coreIndex]: failed to resolve cidWithHint[coreIndex]: not found") //nolint:lll
	})
	...
	t.Run("error - protocol client error", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Empty(t, origin)
		require.Contains(t, err.Error(), "failed to get client versions for namespace [namespace]")
	})
